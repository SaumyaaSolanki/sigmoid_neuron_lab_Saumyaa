...
def train_test_split(features, labels, seed_value):
    ...
    # 3. Calculate the 70/20/10 split for Class 05
...
def activation_function(z):
    prediction = 1/(1 + np.exp(-z)) # what is .exp?
    return prediction
...
def sigmoid(path, learning_rate, epochs, label):
    ...
    for epoch in range(epochs):
        ...
        all_changes = [] # Keeps track of all the changes to my weights and bias for this epoch, so I can check if they stop changing (convergence) or not.
        for change_row in weightChangeList: # For each row in the weightchangelist which keeps track of the changes for each weight
            for change_value in change_row: # For each change value in the row, which is the change for a specific weight or bias
                all_changes.append(change_value) # It appends it to the all_changes list, which is a flat list of all the changes for this epoch, so I can easily find the max change and check for convergence.
                
        # --- THE GUARANTEED 24 STOP ---
       # --- THE EARLY STOPPING BRAKES ---
        current_max_change = max(all_changes) # takes the maximum value out of all the changes in weights AND Bias combined.
        max_weight_change_history.append(current_max_change) # Then it takes the new maximum change and appends to the list that keeps track of all the maximum changes in each epoch, so I can graph it later and see how it goes down over time as the model converges.

        # Trigger stop if weights stop changing (converged)
        if current_max_change < 0.0038:  # If the maximum change in the weights or biases is less than 0.0038, then it will trigger early stopping and stop the training because we don't want to waste time training a model that has already convereged and we don't want to overfit the model on one particular dataset.
            print(f"\n--- Early stopping triggered at Epoch {epoch+1}! ---")
            print("Reason: Reached local minimum (weights stopped changing).")
            break 
            
        # Trigger stop if we hit the maximum allowed epochs
        if (epoch + 1) == epochs: # If we have reached the maximum number of epochs, then it will trigger stopping the training because we don't want to waste time training a model that has already reached the maximum epoch limit and we want to give the user a chance to see the graphs and results before they decide to train again or exit.
            print(f"\n--- Training completed at Epoch {epoch+1}. ---")
            print("Reason: Reached maximum epoch limit.")
            break
        # ---------------------------------
        # ---------------------------------
        # What happens to my weight changes when early stopping is triggered? Do they just stop?
